...
def get_timetree_ics():
    ...
    # 1. Attempt using timetree-exporter CLI if credentials are provided
    if TIMETREE_EMAIL and TIMETREE_PASSWORD:
        try:
            # Run timetree-exporter or internal login to fetch ICS
            cmd = ["timetree-exporter", "-e", TIMETREE_EMAIL, "-p", TIMETREE_PASSWORD, "-c", cal_id]
            res = subprocess.run(cmd, capture_output=True, text=True, timeout=20)
            if res.returncode == 0 and "BEGIN:VCALENDAR" in res.stdout:
                return Response(res.stdout, mimetype="text/calendar", headers={
    ...
    return jsonify({
        "error": f"Could not export calendar {cal_id}. TimeTree blocked public scraping. Please set TIMETREE_EMAIL and TIMETREE_PASSWORD in Render Environment Variables."
    }), 502
