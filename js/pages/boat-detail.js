  ...
  if (main) {
    main.innerHTML = `
      <div class="text-center py-xl">
        ...
        <a href="/ourfleet" class="bg-secondary text-on-secondary px-8 py-3 rounded-lg font-label-md hover:opacity-90 transition-all inline-flex items-center gap-2">
...
function populateBoatDetail(boat) {
  ...
  if (locationEl && boat.location) {
    locationEl.innerHTML = `
      <button class="flex items-center gap-1.5 hover:text-secondary transition-colors cursor-pointer text-left max-w-full whitespace-nowrap overflow-hidden" onclick="window.__showBoatLocationMap('${escapeHtml(boat.name)}', '${escapeHtml(boat.location)}')">
        <span class="material-symbols-outlined text-[16px] md:text-[20px] shrink-0">location_on</span>
        <span class="whitespace-nowrap overflow-hidden">${escapeHtml(boat.location)}</span>
      </button>
    `;
  }
  // --- Quick Specs ---
  ...
  if (pricingEl) {
    ...
    // Check if we have new-style pricing tiers
    const hasNewPricing = pricingTiers.length > 0;
    const hasPricing = hasNewPricing || boatRate > 0 || captainRate > 0;
    ...
    if (hasPricing) {
      ...
      function renderDayPrices(dayCode) {
        ...
        if (hasNewPricing) {
          ...
          if (todayOverrides.length > 0 && dayCode === currentDayName) {
