...
const filePath = 'js/pages/admin-dashboard.js';
let content = fs.readFileSync(filePath, 'utf8');

const target1 = `import { initSocialHub } from '../components/social-hub.js';

document.addEventListener('DOMContentLoaded', async () => {`;

const replace1 = `import { initSocialHub } from '../components/social-hub.js';
...
const target2 = `        const permKeys = ['dashboard','bookings','staff','social','content','fleet','seo','settings','revenue','crm','partners','promos'];
        const permissions = {};
        permKeys.forEach(k => {
...
const replace2 = `        const permissions = {};
...
const target3 = `        const permKeys = ['dashboard','bookings','staff','social','content','fleet','seo','settings','revenue','crm','partners','promos'];
...
const replace3 = `        let grantedMods = 0;
...
const target4 = `    const perms = user.permissions || {};
...
const replace4 = `    const perms = user.permissions || {};
    Object.keys(MODULE_SUBPERMS).forEach(mod => {
      const m = perms[mod];
      const hasAccess = typeof m === 'boolean' ? m : (m?.access ?? false);
      const masterEl = document.getElementById('perm-' + mod + '-access');
...
console.log('Patch applied successfully in node.');
